...
export const DiscordLoginButton: React.FC<DiscordLoginButtonProps> = ({
  ...
  const handleClick = () => {
    ...
    // This would normally redirect the user to Discord's OAuth page
    // For the demo, we'll simulate it with a timeout and fetch a realistic Discord username
    setTimeout(() => {
      // Simulate getting a real Discord username from API
      // In production, this would come from Discord's OAuth response
      ...
      // Set a real-looking Discord username
      localStorage.setItem('username', discordUsernames[Math.floor(Math.random() * discordUsernames.length)]);
      
      onLogin();
      setIsLoading(false);
    }, 1000);
    
    // In production, you would use this line instead:
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isLoading ? 'Connecting...' : 'Authorize with Discord'}
